...
#csa-counties.csv is the U.S. Census Bureau (2023a) data set
#also available on GitHub: https://github.com/dasneddon/IGM
...
#county_rank_slope.csv IS A CLEAN VERSION OF Chetty et al. (2022) DATA
#Available on GitHub: https://github.com/dasneddon/IGM
cbsa <- merge(cbsa, county_rank_slope, by="GEOID")
cbsa$RS <- cbsa$RS*100
##REGRESSION
reg1 <- feols(RS ~ asinh(distance) | densest, 
              data = cbsa, vcov = "hetero")
etable(reg1, tex = TRUE)
#CREATE A CSV FILE FOR cbsa FOR PUBLIC REVIEW
cbsa$geometry <- NULL 
#NECCESSARY FOR CLEAN OUTPUT, GEOMETRIES CAN BE OBTAINED AGAIN USING 
##tigris::counties()
write_csv(cbsa, "cbsa.csv")
#cbsa.csv is the complete data file used in the analysis minus the geometries
#obtained via tigris::counties() available on GitHub: 
#https://github.com/dasneddon/IGM
