...
My interest in this starts with a purely anecdotal personal observation. I spent a combined ten years as a field sales representative and sales manager for a tobacco company. You see all kinds in this line of work and my travels took me into every sort of neighborhood, both rural and urban, wealthy and impoverished, and everything in between. The lions share of our B2B clients were covnenience stores, and nearly every neighborhood in America has a convenience store. One tends to get a sense of the mindset of the managers and clerks that work in these stores. In the urban setting, typically the owners or members of their family are running their stores, often times from open to close. When employees are hired, they typically change from one month to the next. Jobs in these stores are not coveted by the urban labor market. \
...
$\gamma_s : \text{Fixed effects for States}$\
...
The `fixest` package and specifically the `fixest::feols()` function are used for the regression. Standard errors are robust to heteroscedaticity. Fixed effects for U.S. State are chosen to account for the variance in intergenerational mobility by region indicated in @connorstorper.
...
As shown in Table 1, we can reject the null hypothesis, as there is strong evidence that a $1\%$ increase in distance from the most populous county will result in a decrease of $0.1140$ of the rank-rank slope with a high level of significance with $\alpha=0.001$. This indicates that proximity to the most populous county and thus proximity to the most urban area of a CSA has a positive effect on intergenerational mobility.   
...
Overall, it would appear that proximity to an urban area does correlate to relative mobility. This is supported by the high degree of significance in the regression. However, this is ultimately a superficial analysis. My hope is that it can serve as a starting point for more research into the stagnancy and multi-generational poverty of rural areas. I am also curious as to whether work from home trends play a role in revitalizing rural areas, conversely they could also contribute to an increase in income inequality in these areas.
